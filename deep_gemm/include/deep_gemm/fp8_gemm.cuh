...
template <uint32_t SHAPE_N, uint32_t SHAPE_K,
          ...
          GemmType kGemmType>
__global__ void __launch_bounds__(get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
fp8_gemm_kernel_oneshot_scales_b(__nv_bfloat16* gmem_d, float* scales_b, int* grouped_layout,
                uint32_t shape_m,
                const __grid_constant__ CUtensorMap tensor_map_a,
                const __grid_constant__ CUtensorMap tensor_map_b,
                const __grid_constant__ CUtensorMap tensor_map_scales_a,
    ...
    static constexpr uint32_t SHAPE_K_SCALES = ceil_div(SHAPE_K, BLOCK_K);
    static constexpr uint32_t SMEM_SCALES_B_SIZE = ceil_div<uint32_t>(SHAPE_K_SCALES * (kMustUseUniformedScaleB ? 1 : 2) * sizeof(float), sizeof(Barrier)) * sizeof(Barrier);
    // static constexpr uint32_t SMEM_SCALES_B_SIZE_PER_STAGE = sizeof(float);
    ...
    // Fill barriers
    // auto barrier_start_ptr = reinterpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_scales_b) + ceil_div<uint32_t>(SMEM_SCALES_B_SIZE * kNumStages, sizeof(Barrier)) * sizeof(Barrier));
    // auto barrier_start_ptr = reintauto barrier_start_ptr = reinterpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_scales_b) + SMEM_SCALES_B_SIZE);erpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_buffer) + ceil_div<uint32_t>(SMEM_D_SIZE + kNumStages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE + SMEM_SCALES_A_SIZE_PER_STAGE + SMEM_SCALES_B_SIZE_PER_STAGE), sizeof(Barrier)) * sizeof(Barrier));
    auto barrier_start_ptr = reinterpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_scales_b) + SMEM_SCALES_B_SIZE);
    #pragma unroll
    for (int i = 0; i < kNumStages; ++ i) {
        full_barriers[i] = barrier_start_ptr + i;
...
template <uint32_t SHAPE_N, uint32_t SHAPE_K,
          ...
          GemmType kGemmType>
__global__ void __launch_bounds__(get_num_threads_per_sm<kNumTMAThreads, kNumMathThreadsPerGroup>(BLOCK_M), 1)
fp8_gemm_kernel_staged_scales_b(__nv_bfloat16* gmem_d, float* scales_b, int* grouped_layout,
                uint32_t shape_m,
                const __grid_constant__ CUtensorMap tensor_map_a,
                const __grid_constant__ CUtensorMap tensor_map_b,
                const __grid_constant__ CUtensorMap tensor_map_scales_a,
    ...
    // Fill barriers
    // auto barrier_start_ptr = reinterpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_scales_b) + ceil_div<uint32_t>(SMEM_SCALES_B_SIZE * kNumStages, sizeof(Barrier)) * sizeof(Barrier));
    auto barrier_start_ptr = reinterpret_cast<Barrier*>(reinterpret_cast<uint8_t*>(smem_buffer) + ceil_div<uint32_t>(SMEM_D_SIZE + kNumStages * (SMEM_A_SIZE_PER_STAGE + SMEM_B_SIZE_PER_STAGE + SMEM_SCALES_A_SIZE_PER_STAGE + SMEM_SCALES_B_SIZE_PER_STAGE), sizeof(Barrier)) * sizeof(Barrier));
    #pragma unroll
    for (int i = 0; i < kNumStages; ++ i) {
        full_barriers[i] = barrier_start_ptr + i;
        empty_barriers[i] = barrier_start_ptr + kNumStages + i;
            ...
            while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
                launch_k_iterations([&](int k_iter, auto type) {
                    ...
                    for (uint32_t s = 0; s < kNumInnerStages; ++ s) {
                        ...
                        // load your single scale B
                        DG_STATIC_ASSERT(SHAPE_N % 8 == 0, "Invalid shape N");
                        auto local_scale_b = scales_b + (n_block_idx * SHAPE_K_SCALES) + k_idx / BLOCK_K;
                        st_shared(smem_scales_b[s], __ldg(local_scale_b));
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            launch_k_iterations([&](int k_iter, auto type) {
                ...
                for (int s = 0; s < kNumInnerStages; ++ s) {
                    ...
                    // NOTES: even some blocks do not need to read the second row, but we still load one to align with other blocks
                    if constexpr (not kMustUseUniformedScaleB)
                        scale_b_1 = ld_shared(smem_scales_b[s] + SHAPE_K_SCALES);
...
public:
    ...
    static void run(__nv_bfloat16* gmem_d, float* scales_b, int* grouped_layout,
        ...
        auto kernel = fp8_gemm_kernel_oneshot_scales_b<SHAPE_N, SHAPE_K, BLOCK_M, BLOCK_N, BLOCK_K,
